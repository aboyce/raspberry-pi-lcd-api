...
/**
 * @swagger
 * /module/npm/downloads/last-day:
 *  get:
 *    tags:
 *      - npm
 *    summary: Get the last days downloads for a package
 *    parameters:
 *      - in: query
 *        name: name
 *        schema:
 ...
 */
router.get('/downloads/last-day', async (req, res, next) => {
  try {
    const npmPackage = new NpmPackage(req.query.name as string)
    const result = await npmPackage.getLastDayDownloads()
    res.json(result)
    ...
    lcd.printLine(1, `Last Day: ${result.downloads.toLocaleString()}`)
    if (lcd.hasExtraRows()) {
